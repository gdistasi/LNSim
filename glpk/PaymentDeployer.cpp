...
void PaymentDeployer::AddPaymentChannel(int A, int B, double resFundsA,
		double resFundsB, double sendingFeeA, double sendingFeeB, double receivingFeeA, double receivingFeeB) {
	...
	channels.insert(std::map<std::pair<int,int>,PaymentChannel>::value_type(pair<int,int>(A,B),
				    PaymentChannel(A,B,resFundsA,resFundsB, sendingFeeA, sendingFeeB, receivingFeeA, receivingFeeB)));
	...
	if ( B == destination && receivingFeeB != 0){
		std::cerr << "Receiving fee cannot be different from 0 for the destination!n";
		exit(1);
	}

	if ( B==source && sendingFeeB != 0 ){
			std::cerr << "Sending fee cannot be different from 0 for the source!\n";
			exit(1);
	}

	if ( A==destination && receivingFeeA != 0 ){
				std::cerr << "Receiving fee cannot be different from 0 for the destination!\n";
				exit(1);
		}
...
double PaymentDeployer::resFunds(int x,int y){
	...
	if (x<y){
		...
		return channels.find(pair<int,int>(y,x))->second.resFundsB;
	}
}

double PaymentDeployer::sendingFee(int x,int y){
	/* we suppose the payment channels are added following the node id order - HACK */
	if (x<y){
		return channels.find(pair<int,int>(x,y))->second.sendingFeeA;
	} else {
		return channels.find(pair<int,int>(y,x))->second.sendingFeeB;
	}
}

double PaymentDeployer::receivingFee(int x,int y){
	/* we suppose the payment channels are added following the node id order - HACK */
	if (x<y){
...
int  PaymentDeployer::RunSolver(std::vector<std::vector<double>> & flow, double & totalFee) {
